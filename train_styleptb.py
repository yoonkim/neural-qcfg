...
parser.add_argument('--train_file', default='data/StylePTB/ATP/train.tsv')
parser.add_argument('--dev_file', default='data/StylePTB/ATP/valid.tsv')
...
parser.add_argument('--src_pt_states', default=8, type=int)
parser.add_argument('--src_nt_states', default=8, type=int)
parser.add_argument('--dec_dim', default=512, type=int)
parser.add_argument('--dec_char_dim', default=512, type=int)
...
parser.add_argument('--use_copy', default=1, type=int)
# Parser 
...
parser.add_argument('--print_every', type=int, default=500, help='print stats after N examples')
parser.add_argument('--print_trees', type=int, default=1, help='print trees')
parser.add_argument('--eval_every', type=int, default=500, help='eval on dev set after N examples')
...
def main(args):
  ...
  tokenizer = Tokenizer()
  tokenizer.train([d["src"] for d in train_data if len(d["src"]) <= args.sent_max_length_x] + \
                  [d["tgt"] for d in train_data if len(d["tgt"]) <= args.sent_max_length_y],
                  False, args.min_freq)
  ...
  encoder = Encoder(vocab = len(tokenizer.vocab2idx),
                    dim = args.enc_dim,
                    dropout = args.enc_dropout,
                    layers = args.enc_layers,
                    token_type_emb = torch.nn.Embedding(2, args.enc_dim))
  decoder = Decoder(vocab = len(tokenizer.vocab2idx),
                    dim = args.dec_dim,
  ...
  parser = Parser(vocab = len(tokenizer.vocab2idx),
                      dim = args.parser_dim,
    ...
    for d in train_data:
      ...
      with torch.no_grad():
        node_features_argmax, node_spans_argmax = encoder(x_tensor, x_lengths,
                                                          spans = parse_argmax)
        new_spans = []
        for span, x_str in zip(node_spans_argmax, x):
          new_span = []
          for s in span:
            new_span.append([s[0], s[1], x_str[s[0]:s[1]+1]])
          new_spans.append(new_span)
        node_spans_argmax = new_spans
      ...
      if b % args.print_every == 0:
        ...
        log_str = 'Epoch: %d, Batch: %d/%d, |EncParam|: %.4f, |DecParam|: %.4f, ' + \
                  '|ParserParam|: %.4f, LR: %.4f, SrcPPL: %.4f, ' + \
                  'PPL: %.4f, ValPPL: %.4f, ' + \
        ...
        if args.print_trees == 1:
          ...
          with torch.no_grad():
            y_tree, all_spans, all_spans_node = decoder(
              ...
              x_str = y, argmax=True)
          x_idx2vocab = tokenizer.idx2vocab
          y_idx2vocab = tokenizer.idx2vocab
          x_str = [x_idx2vocab[idx] for idx in x_tensor[0].tolist()]
          y_str = [y_idx2vocab[idx] for idx in y_tensor[0].tolist()]
        ...
        if b % args.eval_every == 0:        
          ...
          val_ppl = eval(val_data, encoder, decoder, parser, device, tokenizer)
          print('--------------------------------')
...
def eval(data, encoder, decoder, parser, device, tokenizer):
  encoder.eval()
  decoder.eval()
  parser.eval()
  num_sents = 0
