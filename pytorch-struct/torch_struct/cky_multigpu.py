...
class CKY_MULTIGPU(_Struct):
    def _dp(self, scores, lengths=None, force_grad=False, cache=True):
        ...
        if len(scores) == 3:
            ...
            terms, rules, roots, lse_scores = scores
            add_scores = None
            constraint_scores = None
        elif len(scores) == 5:
            terms, rules, roots, lse_scores, add_scores = scores
            constraint_scores = None
        elif len(scores) == 6:
            terms, rules, roots, lse_scores, add_scores, constraint_scores  = scores
        else:
            assert False
        ...
        curr_dvc = terms.device
        next_dvc = torch.device("cuda:" + str(terms.device.index + 1))
        
        dvc0 = torch.device("cuda:0")
        dvc1 = torch.device("cuda:1")
        dvc2 = torch.device("cuda:2")
        dvc3 = torch.device("cuda:3")
        ...
        for w in range(1, N):
            ...
            Y = beta[A][: N - w, :w, :].to(dvc)
            Z = beta[B][w:, N - w :, :].to(dvc)
            X1 = matmul(matmul(Y.transpose(-2, -1), Z).view(*v2), X_Y_Z.to(dvc))
            all_span.append(X1)

            Y_term = term_use[..., : N - w, :, None].to(dvc)
            Z_term = term_use[..., w:, None, :].to(dvc)

            Y = Y[..., -1, :].unsqueeze(-1)
            X2 = matmul(times(Y, Z_term).view(*v2), X_Y_Z1.to(dvc))

            Z = Z[..., 0, :].unsqueeze(-2)
            X3 = matmul(times(Y_term, Z).view(*v2), X_Y1_Z.to(dvc))
            all_span += [X2, X3]

            if w == 1:
                X4 = matmul(times(Y_term, Z_term).view(*v2), X_Y1_Z1.to(dvc))
                all_span.append(X4)

            span[w] = semiring.sum(torch.stack(all_span, dim=-1)).to(dvc0)
            if add_scores is not None:
                if add_scores[w] is not None:
                    span[w] = span[w] + add_scores[w]
            if lse_scores is not None:
                if lse_scores[w] is not None:
                    span[w] = torch.stack([span[w], lse_scores[w]], 0).logsumexp(0)
            if constraint_scores is not None:
                if constraint_scores[w] is not None:
                    new_score, mask = constraint_scores[w]
                    span[w] = span[w]*(1-mask) + new_score*mask
            beta[A][: N - w, w, :] = span[w]
    ...
    def marginals(self, scores, lengths=None, _autograd=True, _raw=False):
        ...
        v, (term_use, rule_use, root_use, spans), alpha = self._dp(
            scores, lengths=lengths, force_grad=True, cache=False#not _raw
        )
